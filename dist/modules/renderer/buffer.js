export class RendererPathBuffer {
    static initBuffers(gl, path, options, animationFrame) {
        const positions = [];
        const faceColors = [];
        const indices = [];
        ...
        for (let index = 1; index < path.points.length; index++) {
            ...
            // the main tunnel
            {
                ...
                faceColors.push(...[
                    to.color ?? options.leftWallColor?.map((index) => index / 255) ?? options.wallColor?.map((index) => index / 255) ?? [0, 1, 0, 1],
                    to.color ?? options.rightWallColor?.map((index) => index / 255) ?? options.wallColor?.map((index) => index / 255) ?? [0, 0, 1, 1],
            ...
            if (index === path.points.length - 1) {
                ...
                faceColors.push(...[
                    to.color ?? options.endBlockColor?.map((index) => index / 255) ?? options.wallColor?.map((index) => index / 255) ?? [0, 0, 0, 1]
                ]);
                indices.push(...([
                    0, 1, 2,
                    0, 2, 3, // front
                ...
                faceColors.push(...[
                    to.color ?? options.startBlockColor?.map((index) => index / 255) ?? options.wallColor?.map((index) => index / 255) ?? [0, 0, 0, 1]
                ]);
                indices.push(...([
                    0, 1, 2,
                    0, 2, 3, // front
